...
export default function DoctorSubscribeReturn() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (data?.doctorProfile && hasActiveDoctorSubscription(data.doctorProfile)) {
        setStatus("active");
        return;
      }
    ...
    <div className="min-h-screen gradient-surface flex items-center justify-center p-6">
      ...
      {status === "active" && (
        <div className="bg-white rounded-2xl shadow-xl p-10 text-center max-w-md w-full">
          ...
          <p className="text-slate-500 mb-8">Your plan is active — you can see and manage patients again.</p>
      ...
      {status === "pending" && (
        <div className="bg-white rounded-2xl shadow-xl p-10 text-center max-w-md w-full">
          ...
          <div className="flex flex-col gap-2">
            <button onClick={() => router.push("/doctor/subscribe")} className="btn-secondary w-full justify-center py-3">
              Try Again
            </button>
          </div>
        </div>
      ...
      {status === "error" && (
        <div className="bg-white rounded-2xl shadow-xl p-10 text-center max-w-md w-full">
          ...
          <button onClick={() => router.push("/doctor/subscribe")} className="btn-primary w-full justify-center py-3.5">
            Back to Payment
          </button>
        </div>
      )}
